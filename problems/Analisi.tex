...
\Solution{Chiamando $f:\R\to\R$ la funzione analitica definita come
\begin{equation*}
	f(t)=\sum_{n=0}^{\infty} \frac{t^n}{n!^2}
\end{equation*}
è facile ottenere $f'+tf''=(tf')'=f$. 
Da cui si ottiene che la quantità $E(t)=f^2+t{f'}^2$ ha come derivata ${f'}^2$.

Quindi, per ogni $t\le 1$, vale
\begin{equation*}
	f(t)^2 \le E(t) \le E(1) = f(1)^2+ f'(1)^2^2<\frac12 \virgola
\end{equation*}
...
\Problem{Disuguaglianza di Hilbert}{
	Date due sequenze $a_1, a_2, \dots ,a_N$ e $b_1, b_2,\dots, b_N$ di numeri complessi, dimostrare che
	\begin{equation*}
		\left\lvert\sum_{m\not=n}\frac{a_mb_n}{m-n}\right\lvert\le 
		\pi \left(\sum_{m=1}^Na_m^2\right)^{\frac12}\left(\sum_{n=1}^Nb_n^2\right)^{\frac12}\punto
	\end{equation*}
}
\Hint{Considerare l'integrale
\begin{equation*}
	\int_0^{2\pi}(\pi-x)A(-x)B(x)\de x\virgola
\end{equation*}
dove $A$ e $B$ sono i polinomi trigonometrici con coefficienti $(a_m)$ e $(b_n)$.
}

\Problem{Limite di successione per ricorrenza}{
Sia $(a_n)_{n\in\N}$ una successione tale che $a_1 = 0, a_2 = 1$ e per ogni $n\ge 1$ vale
\begin{equation*}
	a_{n+2} = a_{n+1} + \frac{a_n}n\punto
\end{equation*}
Mostrare che esiste il limite per $n\to\infty$ di $\frac{n}{a_n}$ e calcolarlo. 
}
\Hint{Considerare le differenze finite. Magari anche al second'ordine.}
